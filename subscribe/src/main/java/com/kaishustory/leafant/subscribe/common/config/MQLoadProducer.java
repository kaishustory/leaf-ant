...
public class MQLoadProducer {
    ...
    @Primary
    @Bean("loadMQ")
    public Producer createProducer(@Value("${mq.load.groupId}") String groupId, @Value("${ons.access-key}") String accessKey, @Value("${ons.secret-key}") String secretKey, @Value("${mq.addr}") String addr){
        // producer 实例配置初始化
